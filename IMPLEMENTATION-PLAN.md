...
- **Task 48 (firmware `X-Device-Key`) done** — user pasted the real device key into the ESP32 firmware.
- **Task 64's final citation check and Task 35's final chat-answer check both PASS against live production**, closing out the plan. Getting Task 64's check to actually pass took real debugging, not just a quota wait: two initial retests came back as clean declines (`sources:[]`, not an error) against the newly-ingested paper. Root-caused directly (not assumed as "just Gemini flakiness") — the retrieved chunk scored 0.778 against the grounding threshold (retrieval was never the problem), but the test question asked Gemini to *answer* an exam question using content that only *poses* that question (a bare past paper has no model answers) — Gemini was correctly declining per this project's own grounding rule rather than hallucinating. Reframing the question to ask what the paper *asks* (verifiable from the excerpt's literal text) passed cleanly: 200, cited `{Economics, 2024}`, the exact newly-ingested paper. Task 35's last step passed on a plain retry after one transient `503`.
